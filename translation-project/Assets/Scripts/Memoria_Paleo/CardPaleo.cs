...
public class CardPaleo : AbstractScreenReader, ISelectHandler {
    ...
    private GameObject memoryManagerPaleo;
    ...
    private void Start()
    {
        state = VIRADA_BAIXO;
        initialized = false;
        memoryManagerPaleo = GameObject.FindGameObjectWithTag("GameController");
    ...
    {
        cardBack = memoryManagerPaleo.GetComponent<MemoryManagerPaleo>().getCardBack();
        if (choice == MemoryManagerPaleo.CARDFACE)
        {
            cardFace = memoryManagerPaleo.GetComponent<MemoryManagerPaleo>().getCardFace(cardValue);
            cardText = null;
            gameObject.name += ": " + cardFace.name;
            isText = false;
        }
        else if (choice == MemoryManagerPaleo.CARDTEXT)
        {
            cardText = memoryManagerPaleo.GetComponent<MemoryManagerPaleo>().getCardText(cardValue);
            cardFace = null;
